  ...
  program
    ...
    .action(function(){
      self.install(process.cwd()).then(function() {
        console.log('[bower-workspace] Done, happy coding!');
      }).catch(function(err) {
        console.log(err.stack + "\n[bower-workspace] Ooooops, it wasn\'t my fault, I swear");
      });
    });
...
self.log = {
  verbose: function(message) {
    if(program.verbose) {
      console.log("[bower-workspace] " + message);
    }
  },
  ...
  error: function(message) {
    console.error("[bower-workspace] " + message);
  },
...
self.installModule = function(cwd, workspaceDescriptor, packageDescriptor, installed) {
  ...
  self.ensureBowerModules(cwd);
  var nodeModulesDir = path.resolve(cwd, 'bower_components');
  ...
  .then(function() {
    ...
    function processLinked(deps, processed) {
      ...
      _.each(deps, function(version, link) {
        promise = promise.then(function() {
          var linkPackage = require(path.resolve(nodeModulesDir, link, 'bower.json'));
...
/**
 * Launch the bower executable
 */
self.bower = function(args, cwd) {
    var options = {
        cwd: cwd
    };
    if(program.verbose) {
    ...
    return spawned('bower', args, options)
        .catch(function(proc) {
 ...
 */
self.ensureBowerModules = function(cwd) {
  var dir = path.resolve(cwd, 'bower_components');
      ...
      if(mapping) {
        ...
        if(program.copy) {
          ...
          if(!fs.existsSync(dest)) {
            self.log.info("[bower-workspace] Copying "+ dest +" from " + mapping);
            ...
            promise = deferred.promise.then(function() {
              ...
              if(program.production) {
                rimraf.sync((path.join(dest, 'bower_components')));
              }
            });
          }
        ...
        if(!fs.existsSync(dest)) {
          ...
          return self.bower(['install', name+"@"+version], cwd).then(function() {
...
self.getPackageDescriptor = function(cwd, nothrow) {
  var fileDesc = path.resolve(cwd, 'bower.json');
  ...
  if(nothrow) {
    ...
    throw new Error('Cannot find bower.json');
  }
...
self.cleanModule = function(cwd) {
  ...
  rimraf.sync(path.resolve(cwd, 'bower_modules'));
};
